...
router.get('/', function(req, res, next) {
  ...
  pool.getConnection().then(conn =>{
    conn.query("SELECT publicaciones_aprobadas.id_publicacion, publicaciones.titulo, publicaciones.fecha, usuarios.id AS id_creador ,CONCAT( usuarios.nombre, ' ', usuarios.apellido ) AS nombre, ( SELECT COUNT(*) FROM comentarios WHERE comentarios.id_publi = publicaciones_aprobadas.id_publicacion ) AS num_comentarios FROM `publicaciones_aprobadas` INNER JOIN publicaciones ON publicaciones_aprobadas.id_publicacion = publicaciones.id INNER JOIN usuarios ON usuarios.id = publicaciones.id_propietario ORDER BY publicaciones.fecha").then(r => {
